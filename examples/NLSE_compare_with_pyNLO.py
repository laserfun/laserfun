"""Compare the output of the NLSE to that from PyNLO."""
...
# create the pulse
pulse = lf.Pulse(pulse_type='sech', power=1, fwhm_ps=FWHM,
...
fiber_pynlo.generate_fiber(Length * 1e-3, center_wl_nm=pulseWL,
                           betas=(beta2, beta3, beta4),
                           gamma_W_m=Gamma * 1e-3, gvd_units='ps^n/km',
                           gain=-alpha)

evol = pynlo.interactions.FourWaveMixing.SSFM.SSFM(local_error=.0001,
             USE_SIMPLE_RAMAN=True, disable_Raman=np.logical_not(Raman),
             disable_self_steepening=np.logical_not(Steep))

y, AW, AT, pulse_out = evol.propagate(pulse_in=pulse_pynlo, fiber=fiber_pynlo,
                                      n_steps=Steps)
