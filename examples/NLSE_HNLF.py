...
# create the fiber
fiber = lf.Fiber(length, center_wl_nm=pulseWL, dispersion_format='GVD',
                  dispersion=(beta2*1e-3, beta3*1e-3),
                  gamma_W_m=gamma * 1e-3, loss_dB_per_m=alpha*100)

print('Propagation in fiber...')
results = lf.NLSE(pulse_in, fiber, raman=Raman, shock=Steep, nsaves=Steps,
...
ax3.plot(results.z[ind], widths[ind], 'o', color='C1',
...
for pulse, label in zip((pulse_in, shortest, tl), ('Input pulse', 'Shortest pulse', 'Transform limit')):
